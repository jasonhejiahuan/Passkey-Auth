...
| 本地示例和调试 | `/demo/oauth`、`/demo/third-party`、`/demo/link-login` | 用来理解流程，不是生产 UI |

生产环境优先使用标准 OAuth authorization code flow。link challenge flow 更适合你同时控制 Auth 服务和原站 callback 的场景。

## 基础配置

默认配置集中在 `jstu_passkey/config.py`。运行时可用环境变量覆盖：
...
内置示例还要求 `demo=true`。这些条件会在授权完成、code 换 token 和 userinfo
...
示例流程中这些字段存储在 SQLite 表 `oauth_challenge_requests`。
...
当前示例的校验逻辑在 `_consume_challenge_result_token(...)`。如果原网站和 Auth 服务分离部署，推荐两种方式：
...
应用只会在 HTTPS 响应中发送该 `Alt-Svc`，本地 HTTP 示例不会宣告 HTTP/3。
...
- OAuth 授权入口：`jstu_passkey/app.py` 中 `/oauth/authorize`
- OAuth token：`jstu_passkey/app.py` 中 `/oauth/token`
- OAuth userinfo：`jstu_passkey/app.py` 中 `/oauth/userinfo`
- link challenge：`jstu_passkey/app.py` 中 `/oauth/challenge/<challenge_id>`
- 服务端 session verify：`jstu_passkey/app.py` 中 `/api/server/session/verify`
- challenge/code 存储：`jstu_passkey/storage.py`
- WebAuthn 核心逻辑：`jstu_passkey/webauthn_service.py`
- 统一配置：`jstu_passkey/config.py`
